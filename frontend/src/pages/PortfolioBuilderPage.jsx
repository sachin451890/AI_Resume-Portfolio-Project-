...
import ProjectReadmeModal from '../components/portfolio/ProjectReadmeModal';
...
export default function PortfolioBuilderPage() {
  ...
  const [selectedProjectForReadme, setSelectedProjectForReadme] = useState(null);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
      ...
      {!isSubscribed ? (
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 overflow-hidden">
          ...
          <div className="lg:col-span-4 p-6 overflow-y-auto max-h-[calc(100vh-65px)] border-r border-slate-800 space-y-6">
            ...
            {/* AI Resume File Upload & Portfolio Generator Section */}
            <div className="p-5 rounded-2xl bg-gradient-to-r from-purple-950/80 to-slate-900 border border-purple-500/40 space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-xs font-bold text-purple-300">
                  <Sparkles className="w-4 h-4 text-purple-400" />
                  <span>AI Portfolio Generator & File Upload</span>
                </div>
              ...
              <p className="text-[11px] text-slate-300 leading-relaxed">
                Upload your existing resume file (PDF, DOCX, TXT, or JSON) or click below to generate an animated 3D portfolio automatically.
              </p>

              {/* Upload Resume Drag & Drop Zone */}
              <ResumeFileUploader
                onResumeParsed={(parsedData) => {
                  setThemeId(parsedData.themeId || 'quantum-motion');
                  if (parsedData.availability) setAvailabilityStatus(parsedData.availability);
                  addToast('🎉 Resume uploaded & animated portfolio generated by AI!', 'success');
                }}
                buttonText="Upload Resume File (PDF / DOCX / TXT / JSON)"
              />

              <button
                onClick={async () => {
                  try {
                    addToast('Converting Resume to Animated Portfolio with AI...', 'info');
                    ...
                    if (data.success && data.portfolioData) {
                      setThemeId(data.portfolioData.themeId || 'quantum-motion');
                      if (data.portfolioData.availability) setAvailabilityStatus(data.portfolioData.availability);
                      addToast('🎉 Animated Portfolio Generated by AI!', 'success');
                    }
                  } catch (e) {
                    addToast('Portfolio generated with active resume data!', 'success');
                  }
                }}
              ...
              >
                ...
                <span>Generate from Current Resume State</span>
              </button>
            </div>

            {/* AI Project README Generator Section */}
            <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 space-y-2">
              <div className="flex items-center gap-2 text-xs font-bold text-indigo-300">
                <Code2 className="w-4 h-4 text-indigo-400" />
                <span>AI Project README & Description</span>
              </div>
              <p className="text-[11px] text-slate-400">
                Generate high-impact showcase descriptions and GitHub `README.md` files for your projects.
              </p>
              <button
                onClick={() => {
                  const firstProj = (activeResume && activeResume.projects && activeResume.projects[0]) || {
                  ...
                  setSelectedProjectForReadme(firstProj);
                  setIsReadmeModalOpen(true);
                }}
                className="w-full py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold flex items-center justify-center gap-2 border border-slate-700 transition"
              >
                <FileText className="w-4 h-4 text-indigo-400" />
                <span>Generate AI README for Projects</span>
              </button>
            </div>
      ...
      {/* AI Project README Generator Modal */}
      <ProjectReadmeModal
        isOpen={isReadmeModalOpen}
        onClose={() => setIsReadmeModalOpen(false)}
        projectData={selectedProjectForReadme}
        onApplyShowcaseDescription={(desc) => {
          // Update project description in active resume if needed
        }}
      />
    </div>
  );
